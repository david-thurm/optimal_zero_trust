...
#Import the list of role names and new durations
$newRoles = Import-Csv 'Azure_RoleDurationUpdates.csv'
$results = [System.Collections.ArrayList]@()
$newRoles | % {
    $thisNewRole = $_
    $newDuration = $thisNewRole.RoleDuration
    $thisRoleID = $RoleDefinitions | Where displayName -eq $thisNewRole.RoleName | Select id -ExpandProperty id
    $thisRoleID
    ...
    $RolePolicyAssignments | % {
        ...
        if ($RolePolicyRules -ne $null) {
            ...
            $result = [PSCustomObject]@{
                RoleName = $thisNewRole.RoleName
                OriginalExpirationRequired = $RolePolicyRules.isExpirationRequired
                OriginalDuration = $RolePolicyRules.maximumDuration
                NewDuration = $newDuration
                NewRuleStatus = "Unsuccessful"
            }
            $results += $result
        }
        
    
        $RuleUpdateRequestParams = @{
            Method = 'Patch'
            Uri = "https://graph.microsoft.com/v1.0/policies/roleManagementPolicies/$policyID/rules/Expiration_EndUser_Assignment"
            Headers = @{
                "Authorization" = "Bearer $AccessToken"
        ...
        $RuleUpdateRequest = Invoke-WebRequest @RuleUpdateRequestParams
        if ($RuleUpdateRequest.StatusCode -eq 204) {
            $($results | Where RoleName -eq $thisNewRole.RoleName).NewRuleStatus = "Successful"
            Write-Host "$($thisNewRole.RoleName) expiration updated to $newDuration" -ForegroundColor Green
        }
    }
}


$results | Export-Csv "Azure_RoleDurationUpdateResults.csv" -NoTypeInformation
